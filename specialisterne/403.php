<!DOCTYPE html>
<html lang="es">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Acceso Denegado</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="css/styles.css">
    <script src="https://unpkg.com/lucide@latest"></script>
</head>

<body style="background-color: var(--bg-color); display: flex; align-items: center; justify-content: center; height: 100vh;">

    <div class="card text-center" style="max-width: 400px; padding: 40px 20px;">
        <i data-lucide="lock" style="width: 48px; height: 48px; color: var(--text-muted); margin-bottom: 15px;"></i>
        <h1 style="font-size: 20px; margin-bottom: 10px;">Acceso Denegado</h1>
        <p style="color: var(--text-muted); margin-bottom: 25px;">No tienes permisos para acceder a esta sección.</p>
        <a id="logoutBtn" href="index.php" class="btn btn-primary">Volver al login</a>
    </div>
    <script src="js/app.js"></script>
</body>

</html>